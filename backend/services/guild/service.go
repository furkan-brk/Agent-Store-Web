...
// GuildService handles guild CRUD and synergy calculations.
...
func NewGuildService(aiClient *client.AIClient, c *cache.Store) *GuildService {
	return &GuildService{aiClient: aiClient, cache: c}
}

// CreateGuildInput holds the request body for guild creation.
...
// ListGuilds returns a page of guilds.
...
// GetGuild returns a guild with synergy calculations.
func (s *GuildService) GetGuild(id uint) (*models.Guild, []gamification.SynergyBonus, map[string]int, error) {
	var g models.Guild
	err := database.DB.Preload("Members.Agent", func(db *gorm.DB) *gorm.DB {
		return db.Select("id, title, description, service_description, category, creator_wallet, character_type, character_data, subclass, rarity, tags, save_count, use_count, generated_image, price, prompt_score, card_version, created_at, updated_at")
	}).Preload("Members").First(&g, id).Error
	if err != nil {
		return nil, nil, nil, err
	}
	types := make([]string, 0, len(g.Members))
	for _, m := range g.Members {
		if m.Agent.CharacterType != "" {
			types = append(types, m.Agent.CharacterType)
		}
	}
	synergies, bonuses := gamification.CalculateGuildSynergy(types)
	g.Rarity = calculateGuildRarity(g.Members)
	return &g, synergies, bonuses, nil
}

// AddMember adds an agent to a guild.
func (s *GuildService) AddMember(guildID, agentID uint, wallet string) error {
	var g models.Guild
	if err := database.DB.First(&g, guildID).Error; err != nil {
		return errors.New("guild not found")
	}
	if g.CreatorWallet != wallet {
	...
	member := &models.GuildMember{GuildID: guildID, AgentID: agentID, Role: role}
...
// JoinGuild lets any authenticated user join a guild with their first agent.
...
// LeaveGuild removes the user's agent from a guild.
...
// RemoveMember removes an agent from a guild (creator only).
...
// CheckCompatibility analyzes guild member compatibility via AI Pipeline.
func (s *GuildService) CheckCompatibility(guildID uint) (*client.CompatibilityResult, error) {
	var g models.Guild
	if err := database.DB.Preload("Members.Agent", func(db *gorm.DB) *gorm.DB {
		return db.Select("id, title, description, service_description, category, creator_wallet, character_type, character_data, subclass, rarity, tags, save_count, use_count, generated_image, price, prompt_score, card_version, created_at, updated_at")
	}).Preload("Members").First(&g, guildID).Error; err != nil {
		return nil, errors.New("guild not found")
	}

	summaries := make([]client.CompatibilityMember, 0, len(g.Members))
	for _, m := range g.Members {
		summaries = append(summaries, client.CompatibilityMember{
			AgentID:     m.AgentID,
			Title:       m.Agent.Title,
	...
	return s.aiClient.Compatibility(ctx, guildID, summaries)
}
