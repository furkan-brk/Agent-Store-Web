...
RUN set -eux; \
	if [ -f vendor/modules.txt ]; then \
		CGO_ENABLED=0 GOOS=linux go build -mod=vendor -ldflags="-s -w" -o service ./cmd/aipipelinesvc; \
	else \
		CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w" -o service ./cmd/aipipelinesvc; \
	fi
