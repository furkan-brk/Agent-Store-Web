...
RUN set -eux; \
	if [ -f vendor/modules.txt ]; then \
		CGO_ENABLED=0 GOOS=linux go build -mod=vendor -ldflags="-s -w" -o service ./cmd/guildsvc; \
	else \
		CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w" -o service ./cmd/guildsvc; \
	fi
...
USER appuser
EXPOSE 8084
ENV PORT=8084 GOMEMLIMIT=204MiB GOGC=50
CMD ["./service"]
