    ...
    return Obx(() => Scaffold(
      ...
      body: Column(children: [
        Container(
          ...
          child: Row(children: [
            ...
            ShaderMask(
              ...
              child: const Text('Leaderboard', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
            ),
            const Spacer(),
        ...
        if (ctrl.isLoading.value)
          ...
          Expanded(child: Center(child: Text(ctrl.error.value!, style: const TextStyle(color: AppTheme.textB))))
        else if ((ctrl.data.value?['rankings'] as List? ?? []).isEmpty)
          ...
          Expanded(child: ListView.builder(
            ...
            itemCount: (ctrl.data.value!['rankings'] as List).length,
            itemBuilder: (_, i) => _RankCard(entry: (ctrl.data.value!['rankings'] as List)[i] as Map<String, dynamic>),
          )),
      ]),
    ));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: rank <= 3 ? rankColor.withValues(alpha: 0.4) : AppTheme.border),
        gradient: rank == 1 ? LinearGradient(colors: [AppTheme.gold.withValues(alpha: 0.08), Colors.transparent], begin: Alignment.topLeft, end: Alignment.bottomRight) : null,
      ),
      child: Row(children: [
        SizedBox(width: 40, child: rankIcon != null
          ? Icon(rankIcon, color: rankColor, size: 28)
          : Text('#$rank', style: TextStyle(color: rankColor, fontSize: 16, fontWeight: FontWeight.bold))),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(short, style: const TextStyle(color: AppTheme.textH, fontWeight: FontWeight.w600, fontSize: 14, fontFamily: 'monospace')),
          const SizedBox(height: 2),
          Text('$totalAgents agents created', style: const TextStyle(color: AppTheme.textM, fontSize: 11)),
        ])),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Row(children: [const Icon(Icons.bookmark_border, color: AppTheme.primary, size: 13), const SizedBox(width: 3), Text('$totalSaves', style: const TextStyle(color: AppTheme.textB, fontSize: 12))]),
          const SizedBox(height: 3),
          Row(children: [const Icon(Icons.chat_bubble_outline, color: AppTheme.primary, size: 12), const SizedBox(width: 3), Text('$totalUses', style: const TextStyle(color: AppTheme.textB, fontSize: 12))]),
        ]),
      ]),
    );
  }
